    ...
    try:
        ...
        for idx, row in enumerate(all_records[1:], start=2):  # Пропускаем заголовок
            if len(row) > 0 and row[0] == user_id:
                ...
                print(f"   📊 Проверено (F): '{row[5] if len(row) > 5 else 'пусто'}' (len={len(row[5]) if len(row) > 5 else 0})")
                print(f"   📊 Победитель (G): '{row[6] if len(row) > 6 else 'пусто'}' (len={len(row[6]) if len(row) > 6 else 0})")
                ...
                print(f"\n   🎯 РАСПОЗНАНО:")
                print(f"      Статус (очищенный): '{user_status}'")
                print(f"      Победитель (очищенный): '{winner_cell}'")
        ...
        # ЛОГИКА ДЛЯ НЕ-ПОБЕДИТЕЛЕЙ
            ...
            # Неизвестный статус
            print(f"   ⚠️ Неизвестный статус: '{user_status}'")
            bot.send_message(
                message.chat.id,
