...
def check_result(message):
    ...
    try:
      ...
      # Проверяем, есть ли победитель среди записей
        ...
        # Проверяем, есть ли НЕ выигравшие (👎) среди записей
        loser_entry = None
        for entry in user_entries:
            if entry['winner'] == '👎':
                loser_entry = entry
        ...
        # 1️⃣ ЕСЛИ ЕСТЬ ПОБЕДИТЕЛЬ (проверяем ПЕРВЫМ!)
        if winner_entry:
            ...
            if not winner_entry['fullname']:
                ...
                bot.send_message(
                    ...
                    "Пожалуйста, следуйте инструкциям в боте, чтобы мы могли получить ваши данные для отправки подарка. Мы будем хранить их в безопасном месте и никому не передадим.",
                    parse_mode='HTML'
                )
        ...
        # 2️⃣ ЕСЛИ ЕСТЬ ОТМЕТКА "НЕ ВЫИГРАЛ" (и нет победы)
        if loser_entry:
            print(f"   👎 Пользователь НЕ ВЫИГРАЛ (строка {loser_entry['row_index']})")
            bot.send_message(
                message.chat.id,
                "Спасибо за участие в нашей новогодней лотерее!\n\n"
                "😔 К сожалению, в этот раз не повезло.\n"
                "Может быть, попробуете принять участие в следующей волне?\n"
                "Следите за обновлениями в наших соцсетях💙",
                parse_mode='HTML'
            )
            return
        
        # 3️⃣ НЕТ НИ ПОБЕДИТЕЛЯ, НИ ПРОИГРАВШЕГО — ПОКАЗЫВАЕМ СТАТУС ВСЕХ ЗАЯВОК
        status_lines = []
...
def handle_winner_email(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            "Если понадобится что-то уточнить, мы свяжемся с вами через телеграм или почту! 🎁\n\n"
